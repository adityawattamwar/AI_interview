...
const Navigation = ({ view, onViewChange }) => (
...
export const InterviewDashboard = () => {
  ...
  const renderContent = () => {
    ...
    if (step === "setup") {
      return (
        <InterviewSetup
          ...
          onChange={(updates) =>
            setInterviewState({ ...interviewState, ...updates })
          }
    ...
    if (step === "interview" && questions.length > 0) {
      return (
        <Interview questions={questions} onComplete={handleInterviewComplete} />
      );
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-yellow-50 to-teal-50 relative overflow-hidden">
      <GeometricShapes />
      <Header userName={currentUser?.email || ""} onLogout={handleLogout} />
