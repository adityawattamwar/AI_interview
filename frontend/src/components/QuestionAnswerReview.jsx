...
export const QuestionAnswerReview = ({ questions, answers }) => {
  const [expandedIndex, setExpandedIndex] = useState(null);
  const [idealAnswers, setIdealAnswers] = useState({});
  const [loadingIdealAnswer, setLoadingIdealAnswer] = useState(null);

  const getIdealAnswer = async (question, answer, index) => {
    try {
          ...
          {expandedIndex === index && (
            <div className="px-6 py-4 border-t border-yellow-200/50">
              <div className="mb-4">
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html: formatText(answer.question),
                  }}
                />
              </div>
              ...
              {!idealAnswers[index] ? (
                <button
                  onClick={() =>
                    getIdealAnswer(answer.question, answer.answer, index)
                  }
